...
suffix="1kbwindow.out"

echo "working on files with suffix: $suffix"

name="Rhino_1"
echo "Running DESeq2 for $name..."
python3 summarize_multi_count_for_DESeq2.py -n "$name" -s "$study" -c "SRR1187952.${suffix}" "SRR1187953.${suffix}" -t "SRR1187954.${suffix}" -o "$name" 
Rscript DESeq2.R "${name}.countData" "${name}.colData" "${name}.csv"

name="Rhino_2"
echo "Running DESeq2 for $name..."
python3 summarize_multi_count_for_DESeq2.py -n "$name" -s "$study" -c "SRR5445322.${suffix}" "SRR5445323.${suffix}" -t "SRR5445324.${suffix}" "SRR5445325.${suffix}" -o "$name" 
Rscript DESeq2.R "${name}.countData" "${name}.colData" "${name}.csv"
